...
def bvp_to_hr_bpm(bvp: np.ndarray, fs_bvp: int = 64) -> np.ndarray:
    """
    Estime un HR (bpm) à partir du BVP via détection de pics simple (sans scipy).
    ...
    # clip final (sécurité)
    hr = np.clip(hr, 40.0, 200.0).astype(np.float32) #pourq
    return hr
...
def subject_to_windows(data: dict, subject_id: int):
    ...
    # PR (bpm) wrist only, aligné sur EDA
